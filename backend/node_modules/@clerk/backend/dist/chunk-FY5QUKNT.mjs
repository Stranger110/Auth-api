...
var API_VERSION = "v1";
var USER_AGENT = `${"@clerk/backend"}@${"1.13.0"}`;
...
${error.getFullMessage()}`
  ...
  async function attemptRefresh(authenticateContext2) {
    const sessionToken = await refreshToken(authenticateContext2);
    const { data, errors } = await verifyToken(sessionToken, authenticateContext2);
    if (errors) {
  ...
  async function handleError(err, tokenCarrier) {
    ...
    if (isRequestEligibleForRefresh(err, authenticateContext, request)) {
      try {
        ...
        return signedIn(authenticateContext, refreshResponse.data, void 0, refreshResponse.sessionToken);
      } catch (error) {
...
//# sourceMappingURL=chunk-FY5QUKNT.mjs.map